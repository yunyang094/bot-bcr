...
async def start(update: Update, context):
    await update.message.reply_text(f"🤖 Bot BCR VIP đang chạy!\n📦 Tổng {len(icons)} icon xịn\n📅 Gõ /lich để xem lịch\n💎 Gõ /xemicon để test icon")
...
async def lich(update: Update, context):
    ...
    if os.path.exists(SCHEDULE_FOLDER):
        ...
        if files:
            msg = f"📁 Trong thu muc {SCHEDULE_FOLDER} co {len(files)} file:\n" + "\n".join([f"• {x}" for x in files[:20]])
            await update.message.reply_text(msg)
            return
    await update.message.reply_text("Chưa có file lịch thi đấu!")
...
def main():
    ...
    print("Bot BCR FULL - chong xung dot - dang chay...")
    # QUAN TRONG: drop_pending_updates=True de het xung dot khi doi token
    try:
        app.run_polling(drop_pending_updates=True, allowed_updates=Update.ALL_TYPES)
    except Conflict:
        print("Bi xung dot, doi 5s thu lai...")
        import time
        time.sleep(5)
        app.run_polling(drop_pending_updates=True, allowed_updates=Update.ALL_TYPES)
